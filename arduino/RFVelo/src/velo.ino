...
//this method let us define user code to signalize a disfunction
...
void returnBike(){
  ...
  if(rf_core->handShake()) {
    uint8_t return_datas[8];
    return_datas[0]=BIKE_ID;
    return_datas[7]=0;
    unsigned char server_response[8];
    bool received_server_response=false;
    while(!received_server_response){
      rf_core->sendPacket(return_datas);
      received_server_response=rf_core->getPacket(server_response);
    }
    //server send ok or ko +bike_id
    printf("server response:%c%c%d",server_response[0],server_response[1],server_response[2]);
    // for(int i=0;i<8;i++){
    ...
    unsigned char okcode[4];
    okcode[0]='O';
    okcode[1]='K';
    okcode[2]=BIKE_ID;
    //sprintf(okcode,"OK%d\0",BIKE_ID);
    //if(strcmp(server_response,okcode)==0){
    bool perm = true;
    for(int i = 0; i<3;i++){
      perm &=(server_response[i]==okcode[i]);
    }
    if(perm){
...
bool canWithdraw() {
  if(rf_core->handShake()) {
    ...
    printf("Sending RFID:");
    rfid.printHexRFID(client_rfid,6);
    printf(" - and user code: %d",user_code_byte);
    printf("\n\r");
...
boolean getUserCode(){
  ...
  while((actual_time-initial_time) < USER_CODE_TIMEOUT && index_key < USER_CODE_LENGTH){
    ...
    else if(buttons_released && (button_state1== LOW ^ button_state2 == LOW))
    {
      ...
      initial_time=millis(); // this instruction avoid the timeout while user is entering code
